...
<nav class="w-full fixed z-50">
  <div class="px-4 md:px-20 py-6 bg-gray-200 flex items-center justify-between">
      ...
      <a class="flex flex-nowrap text-dark" href="/">
          ...
          <span class="font-medium pl-5 inline-block self-center w-4/5 md:w-full md:h-full drop-shadow-lg">Helios</span>
      </a>
      ...
      <div class="hidden md:flex grow items-center justify-end">
          {#each pages as page}
              <a 
                  class="md:block transition-colors ml-10 text-md lg:text-xl hover:text-blue-400' 
                  {location.path == page.path ? 'text-blue-500' : 'text-gray-700'}" 
  ...
  <div class="relative">
      {#if showMenu}
          <div class="md:hidden absolute bg-gray-200 top-0 right-0 w-full flex flex-wrap z-40" transition:slide|local>
              {#each pages as page}
              <a 
                  class="p-2 w-full text-center hover:text-blue-400 transition-colors {location.path == page.path ? 'text-blue-500' : 'text-gray-700'} hover:text-blue-400" 
                  href="{page.path}">{page.name}</a>
              {/each}
          </div>
      {/if}
